...
/**
 * ============================================================
 * ENDPOINT DE TELEMETRIA GPS EN VIVO (1 Hz)
 * Archivo: api/telemetry.php
 * ============================================================
 * Recibe el paquete JSON enviado por el servicio Foreground
 * de la app Android cada segundo.
 */
...
// SI ES GET: Retorna la última posición de todas las unidades activas (para el Dashboard)
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    ");
    $fleet = $stmt->fetchAll();
    echo json_encode(["status" => "ok", "fleet" => $fleet, "timestamp" => date("Y-m-d H:i:s")]);
    exit;
}

// SI ES POST: Ingestión de telemetría enviada por el celular
$input = file_get_contents("php://input");
$data = json_decode($input, true);

if (!$data || !isset($data['device_uid']) || !isset($data['lat']) || !isset($data['lng'])) {
    http_response_code(400);
    echo json_encode(["status" => "error", "message" => "Datos incompletos. Se requiere device_uid, lat y lng."]);
    exit;
}
...
// 1. Buscar o autoregistrar dispositivo
...
// 3. Actualizar la última posición en la tabla de chofer
if ($driverId) {
    $status = 'en_ruta';
    if ($speed > 75) {
...
// 4. Responder al teléfono con confirmación y directivas pendientes
echo json_encode([
    "status" => "ok",
    "ack" => true,
