...
index = index.replace('    <!-- MODAL: SCREENSHOT VIEWER -->', driverModalHtml + "\n    <!-- MODAL: SCREENSHOT VIEWER -->");
...
const driverJs = `
        ...
        async function deleteDriver(id) {
            if (confirm("¿Estás seguro de borrar este conductor? Esto desvinculará sus dispositivos asociados.")) {
        ...
        async function assignDriver(deviceUid) {
            const driverId = prompt("Para asignar, ingresa el ID NUMÉRICO del conductor (lo puedes ver en la pestaña Conductores):");
            if (driverId) {
...
index = index.replace('// 12. Módulo de APKs', driverJs + "\n        // 12. Módulo de APKs");

const oldDeviceButtons = `<button onclick="selectedDeviceUid='\${dev.device_uid}'; openMdmModalFromHud();" class="px-2.5 py-1 bg-slate-200 hover:bg-slate-300 text-slate-800 rounded border border-slate-300 text-xs">
                                Comandos MDM
                            </button>`;

const newDeviceButtons = `<div class="flex gap-1 justify-end">
...
index = index.replace(oldDeviceButtons, newDeviceButtons);
